...
public class Cplex_model {
    ...
    public IloCplex forwardPass_Stage1() throws Exception {
        ...
        for(int i=0;i<Y_i.length;i++){
            Y_i[i] = cplex.numVar(1, Double.MAX_VALUE, "Y"+(i+1));
        }
    ...
    public IloCplex forwardPass_Stage2toT(int t, Solution solution, int curNode) throws Exception{
        ...
        for(int i=0;i<B_it[curNode].length;i++){
            B_it[curNode][i][t-1] = cplex.numVar(0, Double.MAX_VALUE, "B"+(curNode+1)+","+(i+1)+","+t);
        }
        for(int i=0;i<Ie_it[curNode].length;i++){
            Ie_it[curNode][i][t-1] = cplex.numVar(0, Double.MAX_VALUE, "Ie"+(curNode+1)+","+(i+1)+","+t);
        }
        ...
        for(int i=0;i< blocks.size();i++){
            IloNumExpr obj1 = cplex.prod(blocks.get(i).getHold_cost(), Ie_it[curNode][i][t-1]);
            obj1 = cplex.sum(obj1, cplex.prod(blocks.get(i).getBack_cost(), B_it[curNode][i][t-1]));
            obj = cplex.sum(obj, obj1);
        }
        cplex.addMinimize(obj);
        //约束1：t-1周期末的库存与缺货结算
        for(int i=0;i<blocks.size();i++){
            ...
            if(t > 1) {
                ...
                tempV += (solution.Ie_value[solution.path.get(t-2)][i][t-2] - solution.B_value[solution.path.get(t-2)][i][t-2]);
                c1 = cplex.sum(c1, tempV);
            }
            c1 = cplex.diff(c1, scenarioTree.tree.get(t-1).getCurLayer().get(curNode).get(blocks.get(i).getLength()));
            c1 = cplex.sum(c1, cplex.diff(B_it[curNode][i][t-1], Ie_it[curNode][i][t-1]));
            blockStockRanges[curNode][t-1][i] = cplex.addEq(c1, 0, "blockStock"+(curNode+1)+","+t+","+(i+1));
        }
        //约束2：t周期的生产
        for(int i=0;i<rawMaterials.size();i++){
    ...
    public IloCplex forwardPass_StageT_1(Solution solution, int curNode) throws Exception{
        ...
        //建立第T+1阶段模型
        for(int i=0;i<Ie_it[curNode].length;i++){
            Ie_it[curNode][i][T-1] = cplex.numVar(0, Double.MAX_VALUE, "Ie"+(curNode+1)+","+(i+1)+",T+1");
        }
        for(int i=0;i<B_it[curNode].length;i++){
            B_it[curNode][i][T-1] = cplex.numVar(0, Double.MAX_VALUE, "B"+(curNode+1)+","+(i+1)+",T+1");
        }
        IloNumExpr obj = cplex.numExpr();
        for(int i=0;i< blocks.size();i++){
            IloNumExpr obj1 = cplex.prod(blocks.get(i).getHold_cost(), Ie_it[curNode][i][T-1]);
        ...
        for(int i=0;i<blocks.size();i++){
            ...
            if(T > 1) {
                ...
                c1 = cplex.sum(c1, solution.Ie_value[solution.path.get(T-2)][i][T-2] - solution.B_value[solution.path.get(T-2)][i][T-2]);
            }
            c1 = cplex.diff(c1, scenarioTree.tree.get(T-1).getCurLayer().get(curNode).get(blocks.get(i).getLength()));
            c1 = cplex.sum(c1, cplex.diff(B_it[curNode][i][T-1], Ie_it[curNode][i][T-1]));
            blockStockRanges[curNode][T-1][i] = cplex.addEq(c1, 0, "blockStock"+(curNode+1)+","+T+","+(i+1));
    ...
    public Solution forwardPass_Setup(int T, ArrayList<Integer> path) throws Exception{
        ...
        for(int t=1;t<T;t++){
            ...
            for(int i=0;i<blocks.size();i++){
                solution.Ie_value[path.get(t-1)][i][t-1] = cplex.getValue(Ie_it[path.get(t-1)][i][t-1]);
                solution.B_value[path.get(t-1)][i][t-1] = cplex.getValue(B_it[path.get(t-1)][i][t-1]);
            }
            //cplex.end();
    ...
    public void backwardPass(ArrayList<Integer> path, Solution solution, int iter) throws Exception {
        ...
        duals[path.get(T-1)] = models[path.get(T-1)][T].getDuals(blockStockRanges[path.get(T-1)][T-1]);
        ...
        for(int i=0;i<Parameters.nodeNumPerLayer;i++){
            ...
            duals[i] = models[i][T].getDuals(blockStockRanges[i][T-1]);
        }
        //向T周期所有节点模型添加约束
        for(int n=0;n<Parameters.nodeNumPerLayer;n++) {
            ...
            for(int d=0;d<Parameters.nodeNumPerLayer;d++){
                ...
                for(int i=0;i< blocks.size();i++){
                    ...
                    for (int k = 0; k < rawMaterials.size(); k++) {
                        for (int j = 0; j < allPatterns.get(k).size(); j++) {
                            tempF -= a_ijk[k][j][i] * solution.Q_value[path.get(T - 1 - Parameters.leadTime)][k][j][T - 1 - Parameters.leadTime];
                        }
                    }
        ...
        for(int t=T;t>2;t--) {
            ...
            for (int i = 0; i < Parameters.nodeNumPerLayer; i++) {
                ...
                for(int j=0;j< blocks.size();j++){
                    System.out.println(models[i][t-1].getValue(Ie_it[i][j][t-2]));
                    System.out.println(models[i][t-1].getValue(B_it[i][j][t-2]));
                }
                stockDuals[i] = models[i][t - 1].getDuals(blockStockRanges[i][t - 2]);
            ...
            for (int n = 0; n < Parameters.nodeNumPerLayer; n++) {
                ...
                for (int d = 0; d < Parameters.nodeNumPerLayer; d++) {
                    ...
                    for (int i = 0; i < blocks.size(); i++) {
                        IloNumExpr tempF = cplex.numExpr();
                        tempF = cplex.sum(tempF, cplex.diff(Ie_it[n][i][t - 3], B_it[n][i][t - 3]));
                ...
                for(int d=0;d<Parameters.nodeNumPerLayer;d++){
                    ...
                    for(int i=0;i< blocks.size();i++){
                        ...
                        for (int k = 0; k < rawMaterials.size(); k++) {
                            for (int j = 0; j < allPatterns.get(k).size(); j++) {
                                tempF -= a_ijk[k][j][i] * solution.Q_value[path.get(t-2-Parameters.leadTime)][k][j][t-2-Parameters.leadTime];
                            }
                        }
                    ...
                    for(int it=0;it<iter;it++){
                        temp3 += cutDuals[d][it] * RHS_Value[n][t-2][it];
                    }
        ...
        for (int i = 0; i < Parameters.nodeNumPerLayer; i++) {
            ...
            stockDuals[i] = models[i][1].getDuals(blockStockRanges[i][0]);
        ...
        for(int d=0;d<Parameters.nodeNumPerLayer;d++){
            ...
            for(int i=0;i< blocks.size();i++){
                ...
                for (int k = 0; k < rawMaterials.size(); k++) {
                    for (int j = 0; j < allPatterns.get(k).size(); j++) {
                        tempF -= a_ijk[k][j][i] * solution.Q_value[0][k][j][0];
                    }
                }
                tempF += scenarioTree.tree.get(0).getCurLayer().get(d).get(blocks.get(i).getLength());
                tempF *= stockDuals[d][i];
                temp += tempF;
            }
            rhs += temp / Parameters.nodeNumPerLayer;
            double temp3 = 0;
            for(int it=0;it<iter;it++){
                temp3 += cutDuals[d][it] * RHS_Value[0][0][it];
            }
            rhs += temp3 / Parameters.nodeNumPerLayer;
        }
    ...
    public Cplex_model(ArrayList<Material> rawMaterials, ArrayList<Block> blocks, int T, ScenarioTree scenarioTree){
        this.T = T;
        ...
        this.Ie_it = new IloNumVar[Parameters.nodeNumPerLayer][][];   //[1,T]
        this.Q_ijt = new IloNumVar[Parameters.nodeNumPerLayer][][][];   //[0,T-1]
        this.B_it = new IloNumVar[Parameters.nodeNumPerLayer][][];   //[1,T]
        this.F = new IloNumVar[Parameters.nodeNumPerLayer][T];  //[0,T-1]
        this.blockStockRanges = new IloRange[Parameters.nodeNumPerLayer][][];  //[1,T]
        ...
        for(int i=0;i<Parameters.nodeNumPerLayer;i++){
            ...
            this.blockStockRanges[i] = new IloRange[T][];
            ...
            for(int j=0;j<blocks.size();j++){
                this.Ie_it[i][j] = new IloNumVar[T];
                this.B_it[i][j] = new IloNumVar[T];
            }
            for(int j=0;j< rawMaterials.size();j++){
